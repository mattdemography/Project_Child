...
#Transform 'Amount' to have a mean of 0 and a std of 1. Useful for creating standard dev. charts
  library(caret)
  means_u$Amount2<-means_u$Amount
  t_amount_u<-caret::preProcess(means_u[,c(1,3)], method=c("center", "scale"))
  u_transformed<-predict(t_amount_u, means_u[,c(1,3)])
  means_u<-cbind(means_u, u_transformed[,1])
  colnames(means_u)[4] <- c("Amount_sd")
  rm(t_amount_u, u_transformed)
  
  means_l$Amount2<-means_l$Amount
  t_amount_l<-caret::preProcess(means_l[,c(1,3)], method=c("center", "scale"))
  l_transformed<-predict(t_amount_l, means_l[,c(1:3)])
  means_l<-cbind(means_l, l_transformed[,1])
  colnames(means_l)[4]<-c("Amount_sd")
  rm(t_amount_l, l_transformed)
...
plot_ly(conf_u, x=~Month, y=mean_u, type='scatter', mode='lines', name="Average Amount (Income)", line=list(color='rgb(58,220,225)'), showlegend=F, hoverinfo='text', text="") %>% 
  add_trace(y=~mean, type="bar", marker=list(color=cols_fill, line=list(color=cols_outline, width=2)), showlegend=F,
            error_y=~list(type="data", symmetric=F, array(conf_u$std_error), arrayminus(conf_u$std_error)), color='rgb(65,105,225)', hoverinfo='text', text=paste0(conf_u$mean, " +/-", conf_u$std_error)) %>%
